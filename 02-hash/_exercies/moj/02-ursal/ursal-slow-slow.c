...
void quicksort(candidate_t* v, int lo, int hi) {
  ...
  exch(v[(lo+hi)/2], v[hi-1]);
  cmpexch(v[lo], v[hi-1]);
  cmpexch(v[lo], v[hi]);
  cmpexch(v[hi-1], v[hi]);

  int piv = partition(v, lo+1, hi-1);
  quicksort(v, lo, piv-1);
  quicksort(v, piv+1, hi);
}

void quicksort_wrapper(candidate_t* v, int lo, int hi) {
  quicksort(v, lo, hi);
