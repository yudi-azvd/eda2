...
int main() {
  ...
  mergesort_wrapper(senators, 0, n_eligible_senators-1, &cmpbyvotes_desc);
  print_winners(senators, n_eligible_senators);

  quickselect(depfeds, n_eligible_depfeds, 0, DEPFEDS_CAPACITY-1);
  mergesort_wrapper(depfeds, 0, n_eligible_depfeds-1, &cmpbyvotes_desc);
  print_winners(depfeds, n_eligible_depfeds);

  quickselect(depests, n_eligible_depests, 0, DEPESTS_CAPACITY-1);
  mergesort_wrapper(depests, 0, n_eligible_depests-1, &cmpbyvotes_desc);
